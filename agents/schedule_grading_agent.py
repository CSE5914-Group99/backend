...
# Helper function to create consistent string keys for class_scores dictionary
def create_class_key(class_id: str, teacher: Optional[str]) -> str:
    """
    Create a string key for the class_scores dictionary.
    Format: "{class_id}|{teacher}"

    Args:
        class_id: Normalized class ID (lowercase, no spaces)
        teacher: Normalized teacher name (lowercase, no spaces), or None

    Returns:
        String key in format "class_id|teacher" (e.g., "cse2331|smith" or "math1151|unknown")
    """
    teacher_str = teacher if teacher else "unknown"
    return f"{class_id}|{teacher_str}"
...
class ScheduleScore(BaseModel):
    ...
    class_scores: dict[str, ClassScore] = Field(description="Map of class ID to individual class scores. Keys are in format 'class_id|teacher' (e.g., 'cse2331|smith')")
...
async def score_class_agent_graph(state: ScoreClassInputState) -> dict:
    ...
    # Create string key for the class_scores dictionary
    class_key = create_class_key(state["class_name"], state["teacher_name"])

    # Create a partial ScheduleScore with just this one class
    # This will be merged with other parallel results
    # Use string key in format "class_id|teacher"
    partial_schedule_score = ScheduleScore(
        class_scores={class_key: class_score},
...
async def summarize_schedule(state: ScheduleGradingState) -> ScheduleGradingState:
    ...
    class_summaries = []
    for class_key, cs in class_scores_dict.items():
        # Parse the string key format "class_id|teacher"
        class_id, teacher = class_key.split("|", 1)

        # Format class display name with teacher if available
        if teacher and teacher != "unknown":
            class_display = f"{class_id} (Prof. {teacher})"
        else:
