...
# Pydantic model for ClassScore with metadata (class_id and teacher)
class ClassScoreWithMetadata(BaseModel):
    """ClassScore with class identification metadata"""
    class_id: str = Field(description="The class ID (e.g., 'cse2331')")
    teacher: Optional[str] = Field(description="The teacher name (e.g., 'smith') or 'unknown' if not specified")
    ...
    @classmethod
    def from_class_score(cls, class_score: ClassScore, class_id: str, teacher: Optional[str]) -> "ClassScoreWithMetadata":
        """Create ClassScoreWithMetadata from a ClassScore object"""
        return cls(
            class_id=class_id,
            teacher=teacher or "unknown",
            **class_score.model_dump()
        )
...
class ScheduleScore(BaseModel):
    ...
    class_scores: list[ClassScoreWithMetadata] = Field(description="List of individual class scores with class_id and teacher metadata")
...
async def score_class_agent_graph(state: ScoreClassInputState) -> dict:
    ...
    # Create ClassScoreWithMetadata from the ClassScore
    class_score_with_metadata = ClassScoreWithMetadata.from_class_score(
        class_score=class_score,
        class_id=state["class_name"],
    ...
    partial_schedule_score = ScheduleScore(
        class_scores=[class_score_with_metadata],
...
async def summarize_schedule(state: ScheduleGradingState) -> ScheduleGradingState:
    ...
    class_scores_list = current_schedule_score.class_scores
    ...
    class_summaries = []
    for cs in class_scores_list:
        # Format class display name with teacher if available
        if cs.teacher and cs.teacher != "unknown":
            class_display = f"{cs.class_id} (Prof. {cs.teacher})"
        else:
            class_display = cs.class_id
...
def merge_schedule_scores(left: ScheduleScore | None, right: ScheduleScore | None) -> ScheduleScore | None:
    """Merge schedule scores from parallel executions by combining class_scores lists"""
    if left is None:
        return right
    if right is None:
        return left

    # Merge the class_scores lists from both ScheduleScore objects
